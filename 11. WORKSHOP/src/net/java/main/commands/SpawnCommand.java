...
public class SpawnCommand implements Command {
    ...
    @Override
    public String execute(String[] args) throws InvalidUnitTypeExeption, InvalidPositionException {
        String typeUnit = args[1];
        UnitType unitType;
        try {
            unitType = UnitType.valueOf(typeUnit.toUpperCase());
        } catch (IllegalArgumentException ex) {
            throw new InvalidUnitTypeExeption(
                    ExeptionMessagesConstats.INVALID_UNIT_TYPE_EXCEPTION_MESSAGE
            );
        }
        ...
        return String.format("%s of type %s has spawn @(%d, %d)"
                , unitName
                , unitType.toString()
                , xPosition
                , yPosition);
